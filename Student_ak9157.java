...
public class Student_ak9157 implements Student {
  ...
  double aptitude, List<Double> schools, List<Double> synergies) {
    ...
    for (int i = bottomBound; i < N; i++) {
      School s = new School(i, synergies.get(i) + schools.get(i)); // TODO: Try synergies + qualities as well
      schoolRange[i - bottomBound] = s;
    }
    Arrays.sort(schoolRange);
    
    // paste best choices into ret array
    // TODO: Does arrays.sort() give in reverse order?
    int[] ret = new int[10];
    for (int i = 0; i < 10; i++)
    {
